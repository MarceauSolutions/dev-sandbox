#!/usr/bin/env python3
"""
Generate Comprehensive Medical Review — SSRI Impact, Damage Assessment & Go-Forward Plan.
Branded PDF covering full medication history, SSRI vs castration pharmacology,
developmental impact analysis, and evidence-based path forward.
"""
...
def build_pdf():
    output = os.path.join(os.path.dirname(__file__), "..",
                          ...
                          "comprehensive-medical-review-ssri-impact-and-go-forward-plan.pdf")
    ...
    story.append(Paragraph("& Evidence-Based Go-Forward Plan", cover_title))
    ...
    story.append(Paragraph(
        ...
        "development, comparison to known chemical castration protocols, assessment of reversible "
        "vs. permanent damage, and an evidence-based plan for optimal medication management "
        "going forward.",
        body
    ))
    story.append(Spacer(1, 15))
    ...
    toc = [
        ...
        ["6. Current Medication Comparison: Clonazepam vs. SSRI/NaSSA", "18"],
        ["7. Mirtazapine Transition Analysis — Pittsburgh Decision", "20"],
        ["8. Go-Forward Medication Plan", "21"],
        ["9. Mitigating Current Medication Side Effects", "23"],
        ["10. Reversal & Recovery Protocols for Past SSRI Damage", "24"],
        ["11. Decision Framework — Preventing Future Mistakes", "26"],
    ]
    ...
    # ==================== SECTION 6: CURRENT MED COMPARISON ====================
    story.append(stitle("6. CURRENT MEDICATION COMPARISON"))
    story.append(Paragraph("Clonazepam vs. SSRI/NaSSA for YOUR Situation", h3))
    story.append(Spacer(1, 8))

    story.append(Paragraph(
        "This comparison is specific to William's case: secondary dystonia with pain as the "
        "primary disabling factor, history of SSRI-induced hypogonadism, need for functional "
        "capacity at work, and current age (late 20s — still in testosterone-dependent "
        "development window).",
        body
    ))
    story.append(Spacer(1, 10))
    ...
    comparison = [
        ["Dimension", "Clonazepam\n(Current — 0.5mg BID)", "SSRI (e.g., Lexapro)", "Mirtazapine\n(NaSSA)"],
        [
            "Primary\nMechanism",
            "GABA-A positive allosteric "
            "modulator. Increases chloride "
            "conductance → hyperpolarizes "
            "motor neurons → directly "
            "reduces abnormal muscle firing.",
            "Serotonin reuptake inhibitor. "
            "Increases synaptic 5-HT. Does NOT "
            "directly target motor neurons. "
            "Modulates pain perception via "
            "descending serotonergic pathways.",
            "NaSSA — blocks alpha-2 "
            "adrenergic autoreceptors + "
            "blocks 5-HT2A/2C/3 + potent "
            "H1 antagonist. Mixed "
            "serotonergic/noradrenergic."
        ],
        [
            "Dystonia\nTone",
            "REDUCES TONE — GABAergic "
            "inhibition directly addresses "
            "the excessive motor neuron "
            "firing that causes dystonic "
            "contractions.",
            "DOES NOT REDUCE TONE — "
            "Serotonin does not directly "
            "inhibit motor neurons. May "
            "indirectly reduce tone via "
            "pain-spasm cycle interruption.",
            "MINIMAL TONE EFFECT — "
            "5-HT2A antagonism may have "
            "some indirect benefit but "
            "does not target GABA pathway "
            "that your dystonia responds to."
        ],
        [
            "Pain\nRelief",
            "YES — Reduces pain by reducing "
            "the contractions that cause it, "
            "AND via GABAergic inhibition of "
            "pain signaling in dorsal horn. "
            "Addresses cause AND perception.",
            "PARTIAL — Modulates descending "
            "pain inhibition (serotonergic "
            "pathways). Reduces pain PERCEPTION "
            "but does not address the "
            "contractions causing the pain.",
            "PARTIAL — Some pain modulation "
            "via noradrenergic and "
            "serotonergic pathways. "
            "Did NOT adequately control "
            "pain in William's recent trial."
        ],
        [
            "Testosterone\n/ HPG Axis",
            "MINIMAL IMPACT — GABAergic "
            "drugs do not significantly "
            "affect the HPG axis. "
            "Testosterone and sexual "
            "function are preserved.",
            "SIGNIFICANT SUPPRESSION — "
            "Documented: 300 ng/dL on "
            "40 mg. Prolactin elevation, "
            "LH suppression, testicular "
            "atrophy. Effectively "
            "chemical hypogonadism.",
            "MODERATE RISK — Less sexual "
            "suppression than SSRIs "
            "(blocks 5-HT2A/2C which "
            "cause SSRI sexual effects), "
            "but still elevates prolactin "
            "at higher doses."
        ],
        [
            "Cognitive\nEffects",
            "MILD at low dose — Some "
            "sedation, possible memory "
            "effects. William reports "
            "'limited cognitive impairment.' "
            "Long-term: possible increased "
            "neurodegeneration risk.",
            "POSITIVE in some domains — "
            "William reports enhanced "
            "working memory and learning "
            "ability on SSRIs. Reduced "
            "rumination. However, emotional "
            "blunting reported.",
            "SEDATING — H1 antagonism causes "
            "significant sedation especially "
            "at lower doses (paradoxically "
            "less sedating at higher doses). "
            "Weight gain common."
        ],
        [
            "Personality\nEffects",
            "Slight depression/irritability. "
            "Maintains assertiveness and "
            "independence drive. Can be "
            "more reactive. Maintains "
            "normal male personality traits.",
            "Increased agreeableness. "
            "Reduced dominance/assertiveness. "
            "Emotional blunting. Reduced "
            "risk-taking. 'Really smart "
            "child' — developmentally "
            "suppressed personality.",
            "Sedating, weight gain, "
            "increased appetite. Less "
            "personality flattening than "
            "SSRIs but significant "
            "physical side effects."
        ],
        [
            "Sleep",
            "IMPROVES — GABAergic effect "
            "promotes sleep. Effective for "
            "dystonia-related insomnia.",
            "VARIABLE — Can improve or "
            "worsen sleep. Some patients "
            "report insomnia as side effect.",
            "STRONGLY IMPROVES — H1 "
            "antagonism is powerfully "
            "sedating. Used off-label "
            "as sleep aid."
        ],
        [
            "THC\nDependence",
            "ELIMINATES — Clonazepam "
            "provides sufficient relief "
            "that William reports no need "
            "for THC supplementation.",
            "DOES NOT ELIMINATE — Pain "
            "perception reduced but tone "
            "unchanged. Still needed THC "
            "for breakthrough pain.",
            "DOES NOT ELIMINATE — "
            "Recent trial: still heavily "
            "reliant on THC for pain "
            "management."
        ],
        [
            "Long-Term\nRisks",
            "Dependence. Possible "
            "neurodegeneration (OR 1.5-2x "
            "in observational studies). "
            "Withdrawal danger if stopped "
            "abruptly. Tolerance possible.",
            "HPG axis suppression. Sexual "
            "dysfunction. Delayed physical "
            "maturation. Emotional blunting. "
            "Withdrawal symptoms after "
            "chronic use (brain zaps, "
            "rebound anxiety).",
            "Weight gain (significant). "
            "Metabolic syndrome risk. "
            "Less sexual suppression "
            "but still present at "
            "higher doses."
        ],
    ]
    story.append(btable(comparison[0], comparison[1:],
                        col_widths=[1.1*inch, 1.7*inch, 1.7*inch, 2.3*inch]))
    story.append(Spacer(1, 8))

    story.append(Paragraph(
        "VERDICT: For William's specific case — secondary dystonia where pain AND tone are "
        "the primary complaints, documented history of severe SSRI-induced hypogonadism, "
        "and need for functional capacity with preserved masculinity — clonazepam is the "
        "superior choice despite its long-term risks. SSRIs treat the wrong target (pain "
        "perception vs. motor neuron excitability) while simultaneously causing the most "
        "harmful side effect profile for this specific patient.",
        green_callout
    ))

    story.append(PageBreak())

    # ==================== SECTION 7: MIRTAZAPINE TRANSITION ====================
    story.append(stitle("7. MIRTAZAPINE TRANSITION — PITTSBURGH ANALYSIS"))
    story.append(Spacer(1, 8))
    ...
    # ==================== SECTION 8: GO-FORWARD PLAN ====================
    story.append(stitle("8. GO-FORWARD MEDICATION PLAN"))
    story.append(Spacer(1, 8))

    story.append(Paragraph(
        "Based on the complete medication history, documented responses, and pharmacological "
        "analysis, here is the evidence-based plan for medication management going forward. "
        "The guiding principles are: (1) never repeat the mistakes of the past, (2) preserve "
        "testosterone and physical development, (3) manage pain and tone effectively, "
        "(4) minimize long-term risk, (5) maintain consistency.",
        body
    ))
    story.append(Spacer(1, 10))

    story.append(Paragraph("IMMEDIATE (Current — Month 3)", h3))
    story.extend(blist([
        "CONTINUE clonazepam 0.5 mg BID (1 mg/day total) — this is the lowest effective dose "
        "and is well below typical dystonia dosing (2-6 mg/day). It is working.",
        "Do NOT switch to an SSRI or mirtazapine. The evidence from YOUR body shows these drugs "
        "suppress your HPG axis and do not adequately address dystonia tone.",
        "Address clonazepam-associated depression: consider low-dose bupropion (Wellbutrin) "
        "as adjunct — it is one of the ONLY antidepressants that does NOT cause sexual side "
        "effects (it's actually pro-dopaminergic, which may counteract benzo-induced lethargy). "
        "Discuss with prescriber.",
        "Establish care with a MOVEMENT DISORDER NEUROLOGIST (not a general neurologist or "
        "psychiatrist). Nearest major centers: USF Tampa, University of Miami, Mayo Jacksonville.",
        "Get baseline blood work: testosterone (total + free), prolactin, LH, FSH, SHBG, "
        "comprehensive metabolic panel. This establishes your current hormonal baseline on clonazepam "
        "and serves as comparison for any future medication changes.",
    ]))
    story.append(Spacer(1, 8))

    story.append(Paragraph("SHORT-TERM (Months 3-6): Add Complementary Treatments", h3))
    story.extend(blist([
        "Discuss adding gabapentin or pregabalin specifically for the pain component. These "
        "drugs target calcium channels (different from GABA-A), have evidence for neuropathic pain, "
        "and may allow further reduction of clonazepam dose over time",
        "Discuss trihexyphenidyl — the 2024 RCT (PubMed: 38945037) showed clonazepam + trihexyphenidyl "
        "significantly outperformed trihexyphenidyl alone for dystonia severity, pain, and upper limb function",
        "Start Low-Dose Naltrexone (LDN) 1.5-4.5 mg/day as adjunct — 64% response rate in chronic pain, "
        "novel anti-inflammatory mechanism via microglial modulation, minimal side effects, no dependence risk. "
        "Takes 1-3 months for effect",
        "CBD topicals for localized application to shoulder/trap/arm — zero cognitive impairment, "
        "zero HPG impact, can use during work",
        "Begin acupuncture trial (8-12 sessions) — moderate evidence for dystonia, every participant "
        "in published studies reported improvement including pain reduction",
    ]))
    story.append(Spacer(1, 8))

    story.append(Paragraph("MEDIUM-TERM (Months 6-12): Optimize & Potentially Reduce Benzo", h3))
    story.extend(blist([
        "Assess which complementary treatments are providing benefit",
        "If gabapentin/pregabalin + LDN are contributing meaningfully to pain control, discuss "
        "VERY gradual clonazepam dose reduction (5-10% every 2-4 weeks per 2025 guidelines) "
        "to find the minimum effective dose",
        "Pursue TMS clinical trials — Duke University has active trials for focal hand dystonia. "
        "We drafted an inquiry email (docs/duke-tms-inquiry-email.md). TMS is non-invasive and "
        "could potentially address the root cortical excitability problem",
        "If pain has a neuropathic component not responding to other treatments, evaluate "
        "ketamine infusion therapy ($400-800/session, effects last up to 12 weeks per cycle)",
        "Repeat hormone panel to confirm testosterone remains in normal range",
    ]))
    story.append(Spacer(1, 8))

    story.append(Paragraph("LONG-TERM (Year 1+): Sustainable Management", h3))
    story.extend(blist([
        "Goal: the lowest possible clonazepam dose supplemented by non-benzo treatments "
        "that address pain and tone through complementary mechanisms",
        "Continue monitoring via the automated dystonia research digest (execution/dystonia_research_digest.py) "
        "for new treatments, clinical trials, and emerging therapies",
        "Annual hormone panel to track testosterone, prolactin, metabolic markers",
        "Stay engaged with movement disorder neurology — this is a specialty condition "
        "that requires specialist management, not a GP or psychiatrist",
        "NEVER restart an SSRI without documented clinical necessity, informed consent that "
        "includes HPG monitoring, and an explicit discontinuation plan",
    ]))

    story.append(PageBreak())

    # ==================== SECTION 9: MITIGATING CURRENT MED SIDE EFFECTS ====================
    story.append(stitle("9. MITIGATING CURRENT MEDICATION SIDE EFFECTS"))
    story.append(Spacer(1, 8))

    story.append(Paragraph(
        "Clonazepam at 0.5 mg BID is the best current option, but it has side effects that "
        "should be actively mitigated rather than passively accepted.",
        body
    ))
    story.append(Spacer(1, 8))

    mitigation = [
        ["Side Effect", "Why It Happens", "Mitigation Strategy"],
        [
            "Depression /\nLow Mood",
            "GABAergic enhancement can suppress "
            "dopaminergic and noradrenergic tone. "
            "GABA inhibits excitatory circuits that "
            "contribute to mood and motivation.",
            "1. Low-dose bupropion (Wellbutrin 150 mg) — pro-dopaminergic, "
            "no sexual side effects, counteracts GABA-induced lethargy.\n"
            "2. Regular intense exercise — most potent natural antidepressant. "
            "Resistance training in particular boosts dopamine and testosterone.\n"
            "3. Morning sunlight exposure (15-20 min) — sets circadian rhythm, "
            "boosts serotonin naturally without SSRI-level flooding.\n"
            "4. Omega-3 fatty acids (2-4g EPA+DHA/day) — anti-inflammatory, "
            "modest antidepressant effect in meta-analyses."
        ],
        [
            "Irritability /\nReactivity",
            "Paradoxical disinhibition — GABAergic "
            "drugs can reduce inhibitory control over "
            "emotional responses in some individuals. "
            "More common at lower doses.",
            "1. Awareness is the first mitigation — knowing it's pharmacological, "
            "not characterological, helps manage responses.\n"
            "2. Consider slightly adjusting dose timing if irritability peaks "
            "at specific times (may correlate with trough or peak levels).\n"
            "3. Mindfulness practice — even 10 min/day can improve impulse control.\n"
            "4. If severe: discuss dose adjustment with prescriber."
        ],
        [
            "Long-Term\nNeurodegeneration\nRisk",
            "Observational studies show OR 1.5-2x "
            "for dementia with chronic benzo use. "
            "Mechanism unclear — may be GABA-mediated "
            "suppression of hippocampal function, or "
            "confounding (anxious people are already "
            "at higher neurodegeneration risk).",
            "1. NAD+ precursors — NMN (nicotinamide mononucleotide) 500-1000 mg/day or "
            "NR (nicotinamide riboside) 300-600 mg/day. NAD+ is essential for mitochondrial "
            "Complex I (neuronal energy), activates SIRT1/SIRT3 (DNA repair, reduces "
            "neuroinflammation), and fuels PARP (DNA damage repair in neurons). Chronic "
            "GABA-A modulation may increase oxidative stress — NAD+ directly supports the "
            "repair mechanisms. Strongest evidence-based neuroprotective supplement available.\n"
            "2. N-Acetyl Cysteine (NAC) 600-1200 mg/day — glutathione precursor, "
            "neuroprotective in multiple models, may counteract oxidative stress.\n"
            "3. Magnesium L-Threonate 1-2g/day — the only form that crosses BBB, "
            "supports hippocampal function and synaptic density.\n"
            "4. Omega-3 fatty acids (2-4g EPA+DHA/day) — neuroprotective, anti-inflammatory.\n"
            "5. Regular aerobic exercise — strongest evidence for neurogenesis "
            "and hippocampal volume maintenance.\n"
            "6. Cognitive engagement — learning new skills, building business, "
            "intellectual stimulation maintains neural reserve.\n"
            "7. Keep dose at minimum effective level."
        ],
        [
            "Tolerance\nRisk",
            "GABA-A receptor downregulation over "
            "time may reduce clonazepam's efficacy, "
            "leading to dose escalation pressure.",
            "1. Maintain the lowest effective dose — never increase prophylactically.\n"
            "2. Add complementary non-benzo treatments (gabapentin, LDN, acupuncture) "
            "to reduce reliance on clonazepam alone.\n"
            "3. The 950,000-patient study found only 0.35% escalated to high doses — "
            "escalation is NOT inevitable.\n"
            "4. Regular reassessment with movement disorder neurologist."
        ],
        [
            "Dependence\n& Withdrawal\nConcern",
            "Physical dependence develops after "
            "weeks of regular use. GABA-A receptor "
            "adaptations make abrupt discontinuation "
            "dangerous.",
            "1. NEVER stop abruptly — always taper per 2025 guidelines.\n"
            "2. This is not 'addiction' — it is predictable physiological adaptation. "
            "The distinction matters for self-perception and prescriber communication.\n"
            "3. Have a documented taper plan on file with your prescriber "
            "in case of emergency (hospitalization, prescriber change, etc.).\n"
            "4. Keep a small emergency supply (with prescriber knowledge) "
            "so you're never forced into cold turkey by logistics."
        ],
    ]
    story.append(btable(mitigation[0], mitigation[1:],
                        col_widths=[1.2*inch, 1.8*inch, 3.8*inch]))

    story.append(PageBreak())

    # ==================== SECTION 10: REVERSAL PROTOCOLS ====================
    story.append(stitle("10. REVERSAL & RECOVERY PROTOCOLS"))
    ...
    story.append(stitle("11. DECISION FRAMEWORK — PREVENTING FUTURE MISTAKES"))
    story.append(Spacer(1, 8))

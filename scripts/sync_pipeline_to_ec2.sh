...
if [ -n "$newest_py" ] || [ ! -f "$LAST_CODE_SYNC" ]; then
    ...
    if [ $? -eq 0 ]; then
        ssh -i "$EC2_KEY" -o ConnectTimeout=5 "$EC2_HOST" \
            "sudo cp /tmp/clawdbot_handlers.py /tmp/goal_manager.py /tmp/goal_progress.py /tmp/outcome_learner.py /tmp/gmail_api.py $EC2_PA/ && sudo chown clawdbot:clawdbot $EC2_PA/*.py" 2>/dev/null
        if [ $? -eq 0 ]; then

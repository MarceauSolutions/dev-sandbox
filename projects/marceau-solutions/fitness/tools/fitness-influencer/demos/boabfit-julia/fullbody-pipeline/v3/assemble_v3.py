#!/usr/bin/env python3
"""
Assemble v3 Julia Full-Body Video — Production quality.

Improvements over basic assembly:
  1. Whip pan transitions at segment boundaries (3-frame motion blur)
  2. Flash/white burst at exercise entry points
  3. Gym-warm color grade (boosted warmth + contrast)
  4. Animated circle PiP (scales up from 0 on entry, pulses gently)
  5. Larger, bolder captions with pink highlight on key words
  6. Vignette overlay for cinematic feel
  7. Music volume ducks harder during voice, swells during transitions
"""
import os
import sys
import math
...
OUTPUT = str(BASE / 'julia-fullbody-v3.mp4')

# Exercise B-roll videos (generated via Kling 3.0 Omni with character consistency)
BROLL_VIDEOS = {
    'hip-thrust': str(BASE / 'broll-hip-thrust-kling.mp4'),
    'kickback': str(BASE / 'broll-kickback-kling.mp4'),
    'rdl': str(BASE / 'broll-rdl-kling.mp4'),
}
...
# Key words to highlight in pink
...
FPS = 25
CIRCLE_DIAM = 240  # Larger PiP
CIRCLE_BORDER = 6
CIRCLE_X = WIDTH - CIRCLE_DIAM - 24
CIRCLE_Y = HEIGHT - CIRCLE_DIAM - 380
BRAND_PINK = (255, 105, 180)
DARK_BG = (15, 15, 15)
TRANSITION_DURATION = 0.15  # seconds for whip pan
FLASH_DURATION = 0.08       # white flash

# ── Color Grade (Gym Warm) ───────────────────────────────────────────────────

def color_grade_gym_warm(frame):
    """Apply warm gym color grade: boost warmth, contrast, slight saturation."""
    f = frame.astype(np.float32)
    # Boost contrast
    f = (f - 128) * 1.12 + 128
    # Warm shift: boost red/green shadows, reduce blue
    f[:, :, 0] = f[:, :, 0] * 1.06 + 4   # Red boost
    f[:, :, 1] = f[:, :, 1] * 1.02 + 2   # Green slight
    f[:, :, 2] = f[:, :, 2] * 0.94 - 3   # Blue reduce
...
def build_vignette(w, h, strength=0.35):
    """Create a radial vignette mask (darker edges)."""
    Y, X = np.ogrid[:h, :w]
    cx, cy = w / 2, h / 2
...
def apply_vignette(frame):
    """Apply pre-computed vignette to a frame."""
...
def composite_circle_pip(bg_frame, julia_frame, scale=1.0):
    """Place Julia in a pink circle PiP. Scale 0-1 for animated entrance."""
    if scale <= 0.05:
        return bg_frame
    ...
    # Center the scaled circle at the same position
...
def whip_pan_blur(frame, intensity):
    """Apply horizontal motion blur for whip pan effect. Intensity 0-1."""
    if intensity <= 0.01:
        return frame
    blur_px = int(intensity * 80)  # max 80px blur
    if blur_px < 2:
        return frame
    pil = Image.fromarray(frame)
    blurred = pil.filter(ImageFilter.BoxBlur(blur_px))
    # Directional: stretch horizontally
...
def flash_overlay(frame, intensity):
    """White flash effect. Intensity 0-1."""
    if intensity <= 0.01:
        return frame
...
def render_caption_enhanced(text, width, height, progress=1.0):
    """Render caption with pink-highlighted key words and slide-up animation."""
    ...
    # Slide-up animation (first 0.2s of caption)
    ...
    for i, line in enumerate(lines):
        ...
        # Draw word by word (pink highlight for key words)
        words = line.split(' ')
        wx = x
        for word in words:
...
def render_exercise_label(label, width, height):
    """Exercise name label — large, bold, with icon-style background."""
    img = Image.new('RGBA', (width, height), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
...
def main():
    print("=== Julia Full-Body v3 — Production Assembly ===\n")
    ...
    for name, path in BROLL_VIDEOS.items():
        if os.path.exists(path):
            ...
            print(f"  B-roll {name}: {broll_clips[name].size[0]}x{broll_clips[name].size[1]}, {broll_clips[name].duration:.1f}s")
    ...
    # Collect all transition timestamps (overlay boundaries)
    ...
    def make_frame(t):
        ...
        if active_overlay and active_overlay['video'] in broll_clips:
            ...
            # Animated PiP entrance (scale up in first 0.3s of overlay)
        ...
        for tt in transition_times:
            dt = t - tt
            # Whip pan: motion blur around transition point
            ...
            # White flash right at the transition
    ...
    print(f"\n{'=' * 55}")
    print(f"  V3 PRODUCTION — COMPLETE")
    print(f"  Output: {OUTPUT}")
    print(f"  Size: {size_mb:.1f} MB | Duration: {duration:.1f}s")
    print(f"  Resolution: {WIDTH}x{HEIGHT} | Bitrate: 8Mbps")
    print(f"")
    print(f"  Production effects:")
    print(f"    + Whip pan transitions at segment boundaries")
    print(f"    + White flash on exercise entry")
    print(f"    + Gym-warm color grade (warm tones, boosted contrast)")
    print(f"    + Cinematic vignette")
    print(f"    + Animated circle PiP (scale-up + gentle pulse)")
    print(f"    + Pink-highlighted key words in captions")
    print(f"    + Slide-up caption animations")
    print(f"    + Larger 240px PiP circle")
    print(f"{'=' * 55}")

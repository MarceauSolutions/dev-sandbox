...
**Compiled from:** ESCO 9th Edition practice exam misses (Type I, Type II, Type III) + session study notes
**Target:** Pass EPA 608 Universal certification (Core + Type I + Type II + Type III in one sitting)
...
## Revised study time allocation

Given the chart is in your exam toolbox:
...
**Net effect:** Of the 20 questions you missed across all three practice exams, **5 were chart-lookup questions**. With the chart available, you would have only missed 15 — an improvement from 73% to 80% correct without any additional study.
...
4. Continue until pressure equalizes between system and cylinder
...
This was the underlying cause of most recovery vacuum misses. **You must know which refrigerants are in which category WITHOUT thinking about it.**
...
- Cascade systems, ultra-low temperature, modern transcritical CO₂
...
## Refrigerant Family Identification
...
- R-32 + 90 = 122 → 1 C, 2 H, 2 F, 0 Cl → HFC
- R-134a + 90 = 224 → 2 C, 2 H, 4 F, 0 Cl → HFC
...
- **High ODP = chlorine in molecule + long atmospheric lifetime** (UV in stratosphere breaks C-Cl bond → free Cl destroys ozone catalytically)
...
- Refrigerant recovered from disposed appliances (by type)
...
## Refrigeration Cycle Components (FOUR-component cycle)
...
5. **UV-C in stratosphere breaks C-Cl bond** → releases free Cl atom
...
Key facts:
- "Non-condensable" gases like CO₂ ≠ ozone depletion
...
**Important EPA terminology:**
- Type III = **"low-pressure"** (not "vacuum pressure," not "negative pressure")
- "HC" = **Hydrocarbon** (not HCFC) — R-290, R-600a, R-1270
...
6. **Limited pressurization for leak testing** — 10 psig max (because rupture disc is at 15 psig)

## Why the 200-lb threshold exists
...
- Chemical stability (long shelf life, no system corrosion)
- Low toxicity
- Non-flammability

This makes them all **A1** by ASHRAE classification. The trade-off is that the same chemical stability that makes them safe to breathe makes them persist for decades in the atmosphere — high GWP and (for CFCs/HCFCs) high ODP.
...
R-123 has an OEL of 50 ppm — about 8x stricter than the 400 ppm A/B threshold. This is due to chronic exposure studies showing liver effects in occupational settings. It's not acutely poisonous, but long-term exposure has documented health impacts.
...
- **C-F bonds** → strong IR absorbers (would be high GWP if persistent)
...
- Verifying family classification (CFC/HCFC/HFC) when unsure
- Calculating composition for chemistry problems

For HFOs (R-1000+), the rule still works after subtracting the leading 1 (e.g., R-1234yf → 234, then 234+90 = 324). The "1" prefix indicates the C=C double bond.
...
7. **"HC refrigerant in new household fridges"** → R-600a (isobutane)
...
6. **For ASHRAE classification questions**, default to A1 unless the refrigerant is specifically R-32, R-1234yf, R-454B (A2L), R-290/R-600a (A3), R-123 (B1), or R-717 (B2L).

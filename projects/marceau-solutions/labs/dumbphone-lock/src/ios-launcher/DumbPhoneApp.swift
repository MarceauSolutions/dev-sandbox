...
class NotificationManager {
    ...
    func scheduleFocusEnd(at date: Date) {
        let content = UNMutableNotificationContent()
        content.title = "Focus Session Complete"
        content.body = "Great work staying focused. Your session has ended."
...
class SettingsManager: ObservableObject {
    ...
    func startSession() {
        ...
        NotificationManager.shared.scheduleFocusEnd(at: end)
        Haptics.heavy()
    }
    ...
    func endSession() {
        ...
        sessionEndTime = nil
        UserDefaults.standard.removeObject(forKey: "sessionEndTime")
        NotificationManager.shared.cancelFocusEnd()
        Haptics.success()
    }
            ...
            ScrollView(showsIndicators: false) {
                LazyVGrid(columns: [
                    ...
                    ForEach(AppRegistry.all) { app in
                        Button(action: { Haptics.selection(); settings.toggle(app.key) }) {
                            VStack(spacing: 6) {
                                ZStack {
            ...
            VStack(spacing: 8) {
                ...
                HStack(spacing: 12) {
                    ForEach([30, 60, 90, 120], id: \.self) { mins in
                        Button(action: { Haptics.selection(); settings.focusDuration = mins }) {
                            Text("\(mins)m")
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                HStack {
                    Spacer()
                    Button(action: { Haptics.light(); showSettings = true }) {
...
struct AppTileView: View {
    ...
    var body: some View {
        Button(action: { Haptics.light(); openApp(app.urlScheme) }) {
            VStack(spacing: 8) {
                ZStack {
...
struct SettingsView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                HStack {
                    Button(action: { Haptics.light(); showSettings = false }) {
                        HStack(spacing: 6) {
